...
### Decisão arquitetural: módulo Ajuste de estoque (14/08/2026)

Primeiro módulo de auditoria pura do sistema (criar + listar, sem editar/excluir) — diferente de Clientes/Produtos (CRUD sobre o motor genérico) e de Realizar Venda (cabeçalho + itens + pagamentos). Segue o mesmo raciocínio de Realizar Venda: não usa `module_fields`/`module_tabs`, a tela (`src/features/products/StockAdjustPage.tsx`) é feita à mão porque a UX de escolher produto não é um formulário CRUD simples. Ainda assim precisa de uma linha em `modules` (`ajuste-estoque`) só para aparecer em `/permissoes` — sem isso `has_permission('ajuste-estoque', ...)` nunca encontra nada e ninguém acessa a função, nem o Administrador.

- **Tabela**: `stock_adjustments` (`branch_id`, `product_id`, `change` — delta, pode ser negativo —, `reason`, `balance_after`, `created_at`, `created_by`). Dado operacional, isolado por filial, igual a Produtos.
- **Gravação atômica via função RPC** (`adjust_stock`, `security definer`), mesmo padrão de `create_sale`: valida `has_permission('ajuste-estoque', 'create')` e `has_branch_access(branch_id)` manualmente logo no início, atualiza `products.stock = stock + change` (nunca um valor absoluto — evita corrida entre dois ajustes concorrentes) e insere a linha de auditoria na mesma transação. `revoke execute ... from anon` explícito. Não há policy de `insert`/`update` em `stock_adjustments` para o cliente direto — só a função grava; RLS de `select` usa `has_permission('ajuste-estoque', 'view')` e `has_branch_access(branch_id)`.
- **Sem exclusão/estorno**: apagar um registro de auditoria não desfaz a alteração de estoque que ele representa — quebraria a rastreabilidade. Se um dia for necessário desfazer um ajuste, a solução é lançar um novo ajuste inverso (novo registro), não editar/excluir o antigo. O botão "Excluir" foi removido da tela por esse motivo, em vez de só desabilitado.
- **Tela**: reaproveita `ProductPickerPanel` (o mesmo painel de busca/seleção de produto de Realizar Venda) dentro de um modal (`StockAdjustModal.tsx`) — produto + delta + motivo (texto livre, obrigatório). O `DndContext` que envolve o painel precisa do mesmo `activationConstraint: { distance: 6 }` de Realizar Venda, mesmo sem nenhum "drop" de verdade nesta tela: sem isso o próprio dnd-kit intercepta o clique simples do produto como início de arraste e o clique para de disparar o `onAddProduct`.
- **Repositório/hook**: `stockAdjustmentsRepository.ts`/`useStockAdjustmentsData.ts`, no mesmo padrão de `productsRepository.ts`/`useProductsData.ts`. A listagem faz embed de `products`/`profiles` via PostgREST (`code`, `description`, `stock`, `cost_price`, `location`, `sub_location` do produto; `name` do operador) para preencher a ficha da UI mockada original sem precisar de mais uma tabela.
- Testado no navegador: ajuste positivo e negativo no mesmo produto confirmados em `products.stock` e na listagem; usuário sem `can_create` em `ajuste-estoque` fica com o botão desabilitado (a função também recusaria no banco, mesma validação de `create_sale`).
- **Fora de escopo por enquanto**: teste de isolamento entre duas filiais não foi refeito manualmente nesta rodada (mesma implementação de `has_branch_access` já validada em Produtos/Realizar Venda); campo "Local"/"Sub-local" na ficha vêm do cadastro do produto (não existe local por ajuste).
